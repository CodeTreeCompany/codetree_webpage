  ...
  return (
    <section ref={sectionRef} className={styles.pricing}>
      <div className={styles.container}>
        ...
        {/* Toggle Tabs */}
        <div className={`${styles.tabs} ${isVisible ? styles.animateIn : ''}`}>
          <button 
            className={`${styles.tab} ${activeTab === 'saas' ? styles.active : ''}`}
            onClick={() => setActiveTab('saas')}
          >
            <FaCloudUploadAlt /> SaaS (Mensual)
          </button>
          <button 
            className={`${styles.tab} ${activeTab === 'license' ? styles.active : ''}`}
            onClick={() => setActiveTab('license')}
          >
            <FaShieldAlt /> Licencia Única
          </button>
        </div>

        {/* SaaS Plans */}
        {activeTab === 'saas' && (
          <div className={`${styles.plansContainer} ${isVisible ? styles.animateIn : ''}`}>
            {/* Dynamic Calculator */}
            <div className={styles.calculatorCard}>
              <div className={styles.calculatorHeader}>
                <FaChartLine className={styles.calculatorIcon} />
                <h3>Calculadora de Precios</h3>
                <p>Selecciona la cantidad de animales</p>
              </div>
              <div className={styles.calculatorBody}>
                <div className={styles.rangeContainer}>
                  <label>Cantidad de Animales: <strong>{cowCount}</strong></label>
                  <input 
                    type="range" 
                    min="0" 
                    max="2000" 
                ...
                <div className={styles.priceResult}>
                  ...
                  <span className={styles.priceValue}>{getPriceDisplay()}</span>
                  <span className={styles.pricePeriod}>/ mes</span>
                </div>
                <div className={styles.planRecommendation}>
                  <FaStar className={styles.recommendationIcon} />
                  <span>Plan recomendado: <strong>
                    {selectedPlan === 'basic' ? 'Básico' : 
                     selectedPlan === 'standard' ? 'Estándar' : 
              ...
              {saasPlans.map((plan, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className={styles.planHeader} style={{ background: plan.gradient }}>
                    ...
                    <div className={styles.planBody}>
                      <div className={styles.planLimits}>
                        <div className={styles.limitItem}>
                          <FaBuilding /> {typeof plan.farms === 'number' ? `${plan.farms} granjas` : plan.farms}
                        </div>
                        <div className={styles.limitItem}>
                          <FaUsers /> {typeof plan.employees === 'number' ? `${plan.employees} empleados` : plan.employees}
                        </div>
                        <div className={styles.limitItem}>
                          <FaTractor /> {plan.cows} animales
                        </div>
                      </div>
